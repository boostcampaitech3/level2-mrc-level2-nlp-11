...
def main():
    ...
    train_dataset = org_dataset["train"].to_pandas()

    # 토크나이저 수정필요
    train_context, train_query = tokenize_colbert(
        train_dataset, tokenizer, corpus="both"
    )
    ...
    torch.save(
        trained_model.state_dict(), "/opt/ml/input/code/colbert/best_model/colbert.pth"
    )
...
def train(args, dataset, model):
    ...
    for epoch in train_iterator:
        ...
        torch.save(
            model.state_dict(),
            f"/opt/ml/input/code/colbert/best_model/colbert_epoch{epoch+1}.pth",
        )
